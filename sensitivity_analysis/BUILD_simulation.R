...
dt.sim[, type.char := factor(type,levels=c("average","fixse","gls","gls1","robust","proportion.np","proportion.p"),
                             c("pool (average)","pool (se)","pool (gls)", "pool (constrained gls)","pool (robust gls)","proportion (np)","proportion (p)"))]
dt.sim[, target := factor(type,levels=c("average","fixse","gls","gls1","robust","proportion.np","proportion.p"),
                          c("pool","pool","pool","pool","pool","proportion","proportion"))]
dt.sim[, GS := beta]
dt.sim[target == "proportion", GS := as.numeric(beta>0)]
